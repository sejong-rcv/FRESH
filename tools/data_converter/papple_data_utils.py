...
class PappleInstance(object):
    ...
    def __init__(self, line):
        ...
        self.width = data[8]  / 2
        self.length = data[9]  / 2
        self.height = data[10]  / 2
        self.size = np.array([float(data[9]), float(data[8]), float(data[10])])
        
        self.roll = data[11]
        self.pitch = data[12]
        self.yaw = data[13]
        
        self.angle = np.array([self.roll, self.pitch, self.yaw])
        self.box3d = np.concatenate([self.centroid, self.size, self.angle])
        
        self.iou = data[14]
    

class PAPPLEData(object):
    """PApple data.
    ...
    def __init__(self, root_path, split='train', use_v1=False):
        ...
        self.split = split
        self.split_dir = osp.join(root_path, 'papple_trainval')
        self.classes = [
            'apple'
        ]
    ...
    def get_depth(self, idx):
        ...
        depth = sio.loadmat(depth_filename)['data']
        return depth
    ...
    def get_label_objects(self, idx):
        ...
        objects = [PappleInstance(line) for line in lines]
        return objects
            ...
            if has_label:
                ...
                if annotations['gt_num'] != 0:
                    ...
                    annotations['location'] = np.concatenate([
                        (obj.centroid*10).reshape(1, 3) for obj in obj_list
                        if obj.classname in self.cat2label.keys()
                    ],
                                                             axis=0)
                    ...
                    annotations['gt_boxes_upright_depth'] = np.stack(
                        ...
                        axis=0)  # (K,8)
                    annotations['iou'] = np.array([
                            obj.iou for obj in obj_list
                            if obj.classname in self.cat2label.keys()
                    ])
                    annotations['gt_boxes_upright_depth'] *= 10
