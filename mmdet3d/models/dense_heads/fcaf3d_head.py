...
def nms3d_shlee(boxes: Tensor, scores: Tensor, iou_threshold: float) -> Tensor:
...
class FCAF3DHead(BaseModule):
    ...
    def _get_bboxes_single(self, center_preds, bbox_preds, cls_preds, points,
        ...
        """
        mlvl_bboxes, mlvl_scores, mlvl_angles = [], [], []
...
@HEADS.register_module()
class FCAF3DHead_angle(BaseModule):
    """Bbox head of `FCAF3D <https://arxiv.org/abs/2112.00322>`_. Actually here
    we store both the sparse 3D FPN and a head. The neck and the head can not
    be simply separated as pruning score on the i-th level of FPN requires
    classification scores from i+1-th level of the head.
    ...
    def __init__(self,
                 ...
                 test_cfg=None,
                 init_cfg=None):
        super(FCAF3DHead_angle, self).__init__(init_cfg)
        self.voxel_size = voxel_size
        self.pts_prune_threshold = pts_prune_threshold
        self.pts_assign_threshold = pts_assign_threshold
        self.pts_center_threshold = pts_center_threshold
        ...
        self.train_cfg = train_cfg
        self.test_cfg = test_cfg
        self._init_layers(in_channels, out_channels, n_reg_outs, n_classes, n_angle)
    ...
    def _init_layers(self, in_channels, out_channels, n_reg_outs, n_classes, n_angle):
        """Initialize layers.
        ...
        self.angle_conv = ME.MinkowskiConvolution(
            out_channels, n_angle, kernel_size=1, bias=True, dimension=3)
        self.scales = nn.ModuleList(
            [Scale(1.) for _ in range(len(in_channels))])
    ...
    def forward(self, x):
        ...
        Returns:
            list[list[Tensor]]: Predictions of the head.
        """
        center_preds, bbox_preds, cls_preds, angle_preds, points = [], [], [], [], []
        inputs = x
        x = inputs[-1]
        prune_score = None
        for i in range(len(inputs) - 1, -1, -1):
            ...
            out = self.__getattr__(f'out_block_{i}')(x)
            center_pred, bbox_pred, cls_pred, angle_pred, point, prune_score = \
                self._forward_single(out, self.scales[i])
            center_preds.append(center_pred)
            bbox_preds.append(bbox_pred)
            cls_preds.append(cls_pred)
            ...
            points.append(point)
        return center_preds[::-1], bbox_preds[::-1], cls_preds[::-1], angle_preds[::-1], \
            points[::-1]
    ...
    def forward_train(self, x, gt_bboxes, gt_labels, img_metas):
        ...
        Returns:
            dict: Centerness, bbox and classification loss values.
        """
        center_preds, bbox_preds, cls_preds, angle_preds, points = self(x)
        return self._loss(center_preds, bbox_preds, cls_preds, angle_preds, points,
                          gt_bboxes, gt_labels, img_metas)
    ...
    def forward_test(self, x, img_metas):
        ...
        """
        center_preds, bbox_preds, cls_preds, angle_preds, points = self(x)
        return self._get_bboxes(center_preds, bbox_preds, cls_preds, angle_preds, points,
                                img_metas)
    ...
    def _forward_single(self, x, scale):
        ...
        return center_preds, bbox_preds, cls_preds, angle_preds, points, prune_scores

    def _loss_single(self, center_preds, bbox_preds, cls_preds, angle_preds, points,
                     gt_bboxes, gt_labels, img_meta):
        """Per scene loss function.
        ...
        Returns:
            tuple[Tensor]: Centerness, bbox, and classification loss values.
        """
        center_targets, bbox_targets, cls_targets, angle_targets = self._get_targets(
            points, gt_bboxes, gt_labels)
        ...
        pose_angle_preds = angle_preds[pos_inds]
        if pos_inds.sum() > 0:
            pos_points = points[pos_inds]
            pos_angle_preds = angle_preds[pos_inds]
            # angle_targets = gt_bboxes.tensor[:, 6:]
            # import pdb;pdb.set_trace()
            pos_angle_targets = angle_targets.to(points.device)[pos_inds]
    ...
    def _loss(self, center_preds, bbox_preds, cls_preds, angle_preds, points, gt_bboxes,
              gt_labels, img_metas):
        """Per scene loss function.
        ...
        for i in range(len(img_metas)):
            center_loss, bbox_loss, cls_loss, angle_loss = self._loss_single(
                center_preds=[x[i] for x in center_preds],
                bbox_preds=[x[i] for x in bbox_preds],
                cls_preds=[x[i] for x in cls_preds],
        ...
        #     angle_losses = torch.tensor([0], dtype=torch.float32)
        #     angle_losses = torch.mean(angle_losses)
            
        #     return dict(
        #         center_loss=torch.mean(torch.stack(center_losses)),
        #         bbox_loss=torch.mean(torch.stack(bbox_losses)),
        #         cls_loss=torch.mean(torch.stack(cls_losses)),
        #         angle_loss=torch.mean(angle_losses)
        #     )
        try:
            return dict(
            center_loss=torch.mean(torch.stack(center_losses)),
    ...
    def _get_bboxes_single(self, center_preds, bbox_preds, cls_preds, angle_preds, points,
                           img_meta):
        """Generate boxes for a single scene.
        ...
        """
        mlvl_bboxes, mlvl_scores, mlvl_angles = [], [], []
        for center_pred, bbox_pred, cls_pred, angle_pred, point in zip(
                center_preds, bbox_preds, cls_preds, angle_preds, points):
            scores = cls_pred.sigmoid() * center_pred.sigmoid()
            max_scores, _ = scores.max(dim=1)
            ...
            if len(scores) > self.test_cfg.nms_pre > 0:
                _, ids = max_scores.topk(self.test_cfg.nms_pre)
                bbox_pred = bbox_pred[ids]
                angle_pred = angle_pred[ids]
                scores = scores[ids]
                point = point[ids]

            bboxes = self._bbox_pred_to_bbox(point, bbox_pred)
            # import pdb;pdb.set_trace()
            bboxes = torch.cat((bboxes, angle_pred), dim = 1)
            mlvl_bboxes.append(bboxes)
            mlvl_scores.append(scores)
            # mlvl_angles.append(angle_pred)

        # import pdb;pdb.set_trace()
        bboxes = torch.cat(mlvl_bboxes)
        scores = torch.cat(mlvl_scores)
        # angles = torch.cat(mlvl_angles)
        bboxes, scores, labels = self._single_scene_multiclass_nms(
            bboxes, scores, img_meta)
        return bboxes, scores, labels

    def _get_bboxes(self, center_preds, bbox_preds, cls_preds, angle_preds, points,
                    img_metas):
        """Generate boxes for all scenes.
    ...
    def _get_targets(self, points, gt_bboxes, gt_labels):
        ...
        boxes = torch.cat((gt_bboxes.gravity_center, gt_bboxes.tensor[:, 3:]),
                          dim=1)
        if boxes.shape[1] == 9:
            boxes = boxes.expand(n_points, n_boxes, 9)
        else:
            boxes = boxes.expand(n_points, n_boxes, 7)
        points = points.unsqueeze(1).expand(n_points, n_boxes, 3)
        # import pdb;pdb.set_trace()
        ...
        # import pdb;pdb.set_trace()
        bbox_targets = boxes[torch.arange(n_points), min_inds]
        angle_targets = bbox_targets[:, 6:]
        
        if gt_bboxes.with_yaw:
            if bbox_targets.shape[1] == 9:
                bbox_targets = bbox_targets[:, 0:6]
        ...
        cls_targets = torch.where(min_volumes == float_max, -1, cls_targets)
        # import pdb;pdb.set_trace()
        return center_targets, bbox_targets, cls_targets, angle_targets
    ...
    def _single_scene_multiclass_nms(self, bboxes, scores, img_meta):
        ...
        for i in range(n_classes):
            ...
            if yaw_flag:
                if bboxes.shape[1] == 9:
                    # nms_function = nms3d_custom ## (shlee) 주석처리
                    nms_function = nms3d_shlee
                else:
                    nms_function = nms3d
            else:
                class_bboxes = torch.cat(
