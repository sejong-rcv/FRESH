...
class PappleDataset(Custom3DDataset):
    r"""SUNRGBD Dataset.
    ...
    def get_data_info(self, index):
        ...
        if self.modality['use_camera']:
            img_filename = osp.join(
                osp.join(self.data_root, 'papple_trainval'),
    ...
    def get_ann_info(self, index):
        ...
        if info['annos']['gt_num'] != 0:
            ...
            gt_iou_occ = info['annos']['iou'].astype(np.float32)
        else:
    ...
    def evaluate(self,
                 ...
                 out_dir=None,
                 pipeline=None,
                 ckpt_pth=None):## (shlee) ckpt_pth라는 옵션 추가함
        """Evaluate.
        ...
        if isinstance(results[0], dict):
            return super().evaluate(results, metric, iou_thr, logger, show,
                                    out_dir, pipeline,ckpt_pth=ckpt_pth) ## (shlee) ckpt_pth라는 옵션 추가함
